...
import { createProduct, getCategoryList, updateProduct, getProductDetail } from '@/api'
...
export default function ProductAdd() {
  ...
  const [priceTypeIndex, setPriceTypeIndex] = useState(1) // 默认批发价
  ...
  const handleSubmit = async () => {
    ...
    try {
      showLoading({ title: isEdit ? '保存中...' : '添加中...' })
      ...
      showToast({ title: isEdit ? '修改失败' : '添加失败', icon: 'error' })
    } finally {
      hideLoading()
    }
  }
  ...
  return (
    <View className='product-add-page'>
      <View className='form-section'>
        ...
        <View className='form-item switch-item'>
          <Text className='label'>立即上架</Text>
          <Switch
